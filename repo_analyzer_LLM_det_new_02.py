...
# A copyleft or proprietary notice INSIDE a file, regardless of what the root
# LICENSE says. This is the licence risk that actually bites an acquisition:
# an "MIT" repo carrying twenty GPL files is not an MIT repo.
_FILE_LICENSE_RE = re.compile(
    r"(GNU\s+(?:Affero\s+)?General\s+Public\s+License"
    r"|SPDX-License-Identifier:\s*(?:A?GPL|LGPL|SSPL|BUSL|CC-BY-NC)"
    r"|All\s+rights\s+reserved"
    r"|Proprietary\s+and\s+confidential)", re.IGNORECASE)

# Python function definitions, for annotation coverage.
_PY_ANNOTATED = ("ast",)


def _py_annotation_coverage(tree):
    """(annotated, total) functions — annotated means a return type or at least
    one typed argument. Exact, because it reads the AST rather than the text."""
    total = annotated = 0
    for node in ast.walk(tree):
        ...
        # `self`/`cls` are never annotated and should not count against a method.
        typed_arg = any(a.annotation is not None for a in every
                        if a.arg not in ("self", "cls"))
        if node.returns is not None or typed_arg:
            annotated += 1
...
def scan_file_quality(path, content):
    ...
    syntax_valid = None
    annotated_funcs = typed_total = 0
    if path.endswith(".py"):
        try:
            tree = ast.parse(content)
            syntax_valid = True
            # The AST is already built for the syntax check — annotation
            # coverage is free from the same tree.
            annotated_funcs, typed_total = _py_annotation_coverage(tree)
        except (SyntaxError, ValueError, MemoryError, RecursionError):
            syntax_valid = False
    elif "\x00" in content[:4000]:
        syntax_valid = False

    # Minified or generated: one enormous line, or a handful of very long ones.
    # Counted separately because it inflates LoC without adding anything a model
    # can learn from.
    max_len = max(lens) if lens else 0
    minified = max_len > 1000 or (n < 50 and sum(lens) / n > 200)

    # A licence notice in the file's own header (first ~40 lines).
    head = "\n".join(lines[:40])
    file_license = bool(_FILE_LICENSE_RE.search(head))

    # Comments that are not written in a Latin script. Useful for dataset
    # language balance; deliberately named for what it measures.
    nonascii_comments = sum(
        1 for sline in stripped
        if sline.startswith(_COMMENT_PREFIXES)
        and sum(1 for ch in sline if ord(ch) > 127) > len(sline) * 0.3)
    return {
        ...
        "nonascii_comments": nonascii_comments,
    }
...
class GitLabProvider(HttpBase):
    ...
    def project_info(self, project_id):
        ...
        return {
            ...
            # Provenance: a fork is somebody else's work with a new owner. The
            # API says so plainly, and it costs nothing extra to read here.
            "is_fork": "Yes" if proj.get("forked_from_project") else "No",
            "fork_parent": ((proj.get("forked_from_project") or {})
                            .get("path_with_namespace") or ""),
            "archived": "Yes" if proj.get("archived") else "No",
    ...
    def commit_log(self, info, limit):
        ...
        return [{"author": c.get("author_name", ""),
                 ...
                 # Dates make the difference between "500 commits" and
                 # "500 commits pushed in one evening" — see analyze_history.
                 "date": (c.get("created_at") or c.get("committed_date")
                          or "")[:10]}
                for c in commits]
    ...
    def merge_requests(self, info, limit):
        ...
        for m in mrs:
            out.append({
                ...
                "merged_at": (m.get("merged_at") or "")[:10],
            })
        return out
...
class GitHubProvider(HttpBase):
    ...
    def project_info(self, project_id):
        ...
        return {
            ...
            # Provenance: a fork is somebody else's work with a new owner. The
            # API says so plainly, and it costs nothing extra to read here.
            "is_fork": "Yes" if proj.get("fork") else "No",
            "fork_parent": ((proj.get("parent") or {}).get("full_name") or ""),
            "archived": "Yes" if proj.get("archived") else "No",
            "stars": proj.get("stargazers_count", ""),
        }
...
class LocalProvider:
    ...
    def commit_log(self, info, limit):
        ...
        if info.get("no_git"):
            return []
        args = ["log", "--format=%H%x01%an%x01%ae%x01%aI%x01%B%x02"]
        if limit:
        ...
        for chunk in out.split("\x02"):
            ...
            if not chunk.strip():
                continue
            parts = chunk.split("\x01", 4)
            if len(parts) == 5:
                commits.append({"sha": parts[0], "author": parts[1],
                                "email": parts[2], "date": parts[3][:10],
                                "message": parts[4]})
        return commits
...
# ---------------------------------------------------------------------------
# Repository hygiene: can somebody else build and understand this?
#
# Pure path checks plus a handful of small files that are read once. No API
# calls beyond the tree that was already fetched.
# ---------------------------------------------------------------------------

LOCKFILES = ("package-lock.json", "yarn.lock", "pnpm-lock.yaml", "poetry.lock",
             "Pipfile.lock", "Cargo.lock", "go.sum", "composer.lock",
             "Gemfile.lock", "gradle.lockfile", "uv.lock")

BUILD_FILES = ("dockerfile", "docker-compose.yml", "docker-compose.yaml",
               "compose.yml", "compose.yaml", "makefile", "justfile",
               ".env.example", ".env.sample", "setup.py", "setup.cfg",
...
_DOC_DIR_HINTS = ("docs/", "doc/", "documentation/")


def _count_dependencies(name, content):
    """Direct dependencies declared in one manifest. Returns None when the file
    cannot be read as expected — a wrong guess is worse than a blank."""
    try:
        low = name.lower()
        if low == "package.json":
        ...
        if low == "pyproject.toml":
            # Both layouts, without pulling in a TOML parser: PEP 621 arrays
            # and Poetry's table. Counting entries, not resolving them.
            block = re.search(r"^dependencies\s*=\s*\[(.*?)\]", content,
                              re.S | re.M)
            if block:
                return len([x for x in block.group(1).split(",") if x.strip()])
            poetry = re.search(r"\[tool\.poetry\.dependencies\](.*?)(\n\[|\Z)",
                               content, re.S)
            if poetry:
                return sum(1 for line in poetry.group(1).splitlines()
                           if "=" in line and not line.strip().startswith("#"))
            return None
        if low == "go.mod":
            return len(re.findall(r"^\s+[\w./-]+ v", content, re.M)) or None
        if low in ("cargo.toml", "composer.json", "gemfile", "pom.xml"):
            if low == "composer.json":
                data = json.loads(content)
                return len(data.get("require") or {}) + len(
                    data.get("require-dev") or {})
            if low == "pom.xml":
                return len(re.findall(r"<dependency>", content)) or None
            if low == "gemfile":
                return len(re.findall(r"^\s*gem\s+", content, re.M)) or None
            block = re.search(r"\[dependencies\](.*?)(\n\[|\Z)", content, re.S)
            if block:
                return sum(1 for line in block.group(1).splitlines()
                           if "=" in line and not line.strip().startswith("#"))
...
def _analyze_hygiene(prov, info, all_paths, result, content_cache):
    """Dependencies, build files and documentation."""
    print("  Repo hygiene (deps, build, docs)...")
    # Not root-only: a monorepo keeps its Dockerfile in backend/ and its
    # package.json in frontend/, and calling that "no Dockerfile" is simply
    # wrong. Take the SHALLOWEST occurrence of each filename, skipping vendored
    # and dependency directories so node_modules cannot answer for the repo.
    roots = {}
    for path in all_paths:
        ...
        prev = roots.get(name)
        if prev is None or depth < prev.count("/"):
            roots[name] = path

    # ---- lockfiles & build files (path checks only — no reads) ----
    result["has_lockfile"] = "Yes" if any(
        lf.lower() in roots for lf in LOCKFILES) else "No"
    found_build = sorted({b for b in BUILD_FILES if b in roots})
    result["build_files"] = "; ".join(found_build)
    result["has_dockerfile"] = "Yes" if any(
        k == "dockerfile" or k.startswith("dockerfile.") for k in roots) else "No"

    # ---- dependency count from the first manifest we can read ----
    deps, manifest_used = None, ""
    for mf in MANIFESTS:
        path = roots.get(mf.lower())
        if not path:
            continue
        content = _fetch_cached(prov, info, path, content_cache)
        if not content:
            continue
        n = _count_dependencies(mf, content)
        if n is not None:
            deps, manifest_used = n, mf
            break
    result["direct_dependencies"] = deps if deps is not None else ""
    result["dependency_manifest"] = manifest_used

    # ---- pinned versions (requirements.txt only — the one place it is exact) ----
    req = roots.get("requirements.txt")
    if req:
        content = _fetch_cached(prov, info, req, content_cache) or ""
        lines = [l.strip() for l in content.splitlines()
                 if l.strip() and not l.strip().startswith(("#", "-"))]
        if lines:
            result["pct_pinned_deps"] = _pct(
                sum(1 for l in lines if "==" in l), len(lines))
        else:
            result["pct_pinned_deps"] = ""
    else:
        result["pct_pinned_deps"] = ""

    # ---- documentation ----
    readme_path = next((p for k, p in roots.items() if k.startswith("readme")), "")
    words, blocks = "", ""
    if readme_path:
        content = _fetch_cached(prov, info, readme_path, content_cache) or ""
        words = len(content.split())
        blocks = content.count("```") // 2
    result["readme_words"] = words
    result["readme_code_blocks"] = blocks
    result["doc_files"] = sum(
        1 for p in all_paths
        if p.lower().startswith(_DOC_DIR_HINTS) and p.lower().endswith(
            (".md", ".rst", ".txt", ".adoc")))
    result["has_contributing"] = "Yes" if any(
        k.startswith("contributing") for k in roots) else "No"
...
def analyze_project(prov, project_id, max_mrs, sample_mrs,
    ...
    result = {
        ...
        # Provenance, straight from the provider. Blank for a local folder,
        # where there is nothing authoritative to read.
        "is_fork": info.get("is_fork", ""),
        "fork_parent": info.get("fork_parent", ""),
    ...
    # One commit fetch, two readers: LLM detection reads the messages, history
    # analysis reads the dates. Fetching it twice would double the API cost for
    ...
    _analyze_llm_usage(prov, info, mrs, all_paths, result,
                       ...
                       content_cache, commits)
    analyze_history(commits, result)
    analyze_review_depth(mrs, result)
    _analyze_hygiene(prov, info, all_paths, result, content_cache)
...
# ---------------------------------------------------------------------------
# History shape, authorship and review depth
#
# All of this comes from data the scan ALREADY fetched — the commit log and the
# MR/PR list — so it costs no extra API calls. What it answers is the question
# a buyer actually has: was this repository developed, or was it dumped?
# ---------------------------------------------------------------------------

FREE_EMAIL_DOMAINS = {"gmail.com", "yahoo.com", "hotmail.com", "outlook.com",
                      "protonmail.com", "proton.me", "icloud.com", "aol.com",
                      "mail.com", "yandex.com", "rediffmail.com", "live.com"}

# A commit message that says nothing. "fix", "update", "asdf", "."
_WEAK_MSG_RE = re.compile(
    r"^(fix|fixes|fixed|update|updates|updated|change|changes|changed|wip|test|"
    r"tests|temp|tmp|stuff|misc|minor|edit|edits|commit|final|done|ok|asdf|"
...
_ISSUE_REF_RE = re.compile(r"(#\d+|[A-Z][A-Z0-9]+-\d+)")
...
def _days_between(a, b):
    """Whole days between two YYYY-MM-DD strings, or None if either is bad."""
    try:
...
def analyze_history(commits, result):
    """How the code arrived: over months, or in one sitting.

    A repository with 400 commits spread over two years and one with 400
    commits pushed on a single afternoon look identical in every count-based
    metric. They are not the same product. The dates separate them.

    Everything here degrades to blank rather than guessing: an old report, a
    provider that hides dates, or a plain folder with no git simply leaves
    these columns empty.
    """
    dates = sorted(d for d in ((c.get("date") or "")[:10] for c in commits) if d)
    result["commit_span_note"] = ""
    if not dates:
        for k in ("first_commit", "last_commit", "active_days", "active_months",
                  "commits_per_month", "max_gap_days", "pct_commits_busiest_day",
                  "single_drop"):
            result[k] = ""
    else:
        by_day = Counter(dates)
        by_month = Counter(d[:7] for d in dates)
        span_days = _days_between(dates[0], dates[-1]) or 0
        result["first_commit"] = dates[0]
        result["last_commit"] = dates[-1]
        result["active_days"] = len(by_day)
        result["active_months"] = len(by_month)
        result["commits_per_month"] = (round(len(dates) / max(1, len(by_month)), 1))
        # The longest silence, in days — a repo with a 3-year gap is two repos.
        uniq = sorted(by_day)
        gaps = [g for g in (_days_between(uniq[i], uniq[i + 1])
                            for i in range(len(uniq) - 1)) if g is not None]
        result["max_gap_days"] = max(gaps) if gaps else 0
        busiest = by_day.most_common(1)[0][1]
        result["pct_commits_busiest_day"] = _pct(busiest, len(dates))
        # The dump test. Not "few commits" — few DAYS. A real project cannot be
        # written in one or two sittings, whatever the commit count says.
        result["single_drop"] = ("Yes" if len(by_day) <= 2 or busiest / len(dates) > 0.9
                                 else "No")
        if span_days and len(by_day) <= 2:
            result["commit_span_note"] = f"all commits within {len(by_day)} day(s)"
    result["commit_dates_seen"] = len(dates)

    # ---- who wrote it ----
    authors = Counter()
    domains = Counter()
    for c in commits:
        key = (c.get("email") or c.get("author") or "").strip().lower()
        if key:
            authors[key] += 1
    ...
    if total:
        result["top_author_pct"] = _pct(authors.most_common(1)[0][1], total)
        result["commit_authors"] = len(authors)
    else:
        result["top_author_pct"] = ""
        result["commit_authors"] = ""
    result["author_domains"] = len(domains)
    free = sum(n for d, n in domains.items() if d in FREE_EMAIL_DOMAINS)
    result["pct_free_email_commits"] = _pct(free, sum(domains.values())) if domains else ""

    # ---- what the messages say ----
    messages = [(c.get("message") or "").strip() for c in commits]
    subjects = [m.splitlines()[0] if m else "" for m in messages]
    subjects = [x for x in subjects if x]
    if subjects:
        result["median_commit_msg_len"] = _median([len(x) for x in subjects])
        result["pct_weak_commit_msgs"] = _pct(
            sum(1 for x in subjects if _WEAK_MSG_RE.match(x)), len(subjects))
        result["pct_conventional_commits"] = _pct(
            sum(1 for x in subjects if _CONVENTIONAL_RE.match(x)), len(subjects))
        result["pct_commits_with_issue_ref"] = _pct(
            sum(1 for x in subjects if _ISSUE_REF_RE.search(x)), len(subjects))
    else:
        for k in ("median_commit_msg_len", "pct_weak_commit_msgs",
                  "pct_conventional_commits", "pct_commits_with_issue_ref"):
            result[k] = ""


def analyze_review_depth(mrs, result):
    """Was the code reviewed, and how long did review take?

    Review conversation is the most expensive thing in a repository to fake and
    the most valuable thing in it to train on. `notes` is already fetched with
    the MR list on GitLab; on GitHub the list endpoint omits it, so it is only
    known for the MRs that were deep-analyzed — hence the "of N sampled" basis.
    """
    mrs = mrs or []
    with_notes = [m for m in mrs if isinstance(m.get("notes"), int)]
    if with_notes:
        commented = sum(1 for m in with_notes if m["notes"] > 0)
        result["pct_mrs_reviewed"] = _pct(commented, len(with_notes))
        result["avg_review_comments"] = round(
            sum(m["notes"] for m in with_notes) / len(with_notes), 1)
        result["review_basis"] = f"{len(with_notes)}/{len(mrs)} with comment counts"
    else:
        result["pct_mrs_reviewed"] = ""
        result["avg_review_comments"] = ""
        result["review_basis"] = ""

    ttm = [d for d in (_days_between(m.get("created_at", ""), m.get("merged_at", ""))
                       for m in mrs
                       if m.get("merged") and m.get("created_at") and m.get("merged_at"))
           if d is not None]
    result["median_days_to_merge"] = _median(ttm) if ttm else ""
    result["merged_with_dates"] = len(ttm)
...
def _analyze_llm_usage(prov, info, mrs, all_paths, result,
                       max_commit_scan, max_ai_file_scan, workers,
                       content_cache=None, commits=None):
    ...
    # ---- 1. Commits scan (fetched by the caller and shared) ----
    if commits is None:
        try:
            commits = prov.commit_log(info, max_commit_scan) or []
        except Exception as e:
...
def _analyze_training_quality(prov, info, all_paths, result,
    ...
    with ThreadPoolExecutor(max_workers=pool_workers) as pool:
        ...
        for fut in as_completed(futures):
            ...
            tot["emails"] += emails
            tot["annotated_funcs"] += q["annotated_funcs"]
            tot["typed_total"] += q["typed_total"]
            tot["minified_files"] += 1 if q["minified"] else 0
            tot["nonascii_comments"] += q["nonascii_comments"]
            if q["file_license"]:
                license_header_files.append(p)
            if q["hash"]:
                hashes[q["hash"]] += 1
    ...
    result["type_hint_pct"] = _pct(tot["annotated_funcs"], tot["typed_total"])
    result["typed_functions_seen"] = tot["typed_total"]
    result["minified_files"] = tot["minified_files"]
    result["pct_minified_files"] = _pct(tot["minified_files"], files_scanned)
    result["nonascii_comment_pct"] = _pct(tot["nonascii_comments"],
                                          tot["comment_lines"])
    # Only a CONTRADICTION is worth reporting: a copyleft/proprietary header in
    # a repo whose own licence is permissive or absent. In a GPL repo the same
    # headers are simply the licence doing its job.
    repo_risk = (result.get("license_risk") or "").lower()
    contradiction = repo_risk in ("permissive", "unknown",
                                  "no-license (all rights reserved)")
    result["conflicting_license_files"] = (
        len(license_header_files) if contradiction else 0)
    result["conflicting_license_detail"] = (
        license_header_files[:20] if contradiction else [])
...
CSV_COLUMNS = [
    ...
    "Code availability",
    # --- added: provenance ---
    "Is fork", "Fork parent", "Archived", "Stars",
    # --- added: how the code arrived ---
    "First commit", "Last commit", "Active days", "Active months",
    "Commits per month", "Longest gap (days)", "Busiest day % of commits",
    "Single drop",
    # --- added: who wrote it ---
    "Commit authors", "Top author % of commits", "Author email domains",
    "Free-email commits %",
    # --- added: how they wrote it ---
    "Median commit msg length", "Weak commit msgs %", "Conventional commits %",
    "Commits with issue ref %",
    # --- added: review ---
    "MRs reviewed %", "Avg review comments", "Median days to merge",
    "Review basis",
    # --- added: hygiene ---
    "Direct dependencies", "Dependency manifest", "Lockfile", "Dockerfile",
    "Build files", "Pinned deps %", "README words", "README code blocks",
    "Doc files", "CONTRIBUTING",
    # --- added: content quality ---
    "Type hint % (py)", "Minified files %", "Non-ASCII comments %",
...
def aggregate(results, name):
    ...
    for f in ("total_loc_estimate", "commits", "total_mrs", "merged_mrs",
              ...
              "active_days", "active_months", "commit_authors",
              "author_domains", "direct_dependencies", "readme_words",
              "readme_code_blocks", "doc_files", "minified_files",
              "typed_functions_seen", "conflicting_license_files",
              "commit_dates_seen", "merged_with_dates"):
    ...
    # Added metrics. Rates are weighted by files scanned like the rest; dates
    # take the outer bounds; yes/no flags are true if true for any repo.
    for key in ("type_hint_pct", "pct_minified_files", "nonascii_comment_pct"):
        agg[key] = _wavg(key)
    firsts = [r["first_commit"] for r in results if r.get("first_commit")]
    lasts = [r["last_commit"] for r in results if r.get("last_commit")]
    agg["first_commit"] = min(firsts) if firsts else ""
    agg["last_commit"] = max(lasts) if lasts else ""
    for key in ("commits_per_month", "max_gap_days", "pct_commits_busiest_day",
                "top_author_pct", "pct_free_email_commits",
                "median_commit_msg_len", "pct_weak_commit_msgs",
                "pct_conventional_commits", "pct_commits_with_issue_ref",
                "pct_mrs_reviewed", "avg_review_comments",
                "median_days_to_merge", "pct_pinned_deps"):
        vals = [r[key] for r in results if isinstance(r.get(key), (int, float))]
        agg[key] = round(sum(vals) / len(vals), 1) if vals else ""
    for key, yes in (("single_drop", "Yes"), ("is_fork", "Yes"),
                     ("archived", "Yes"), ("has_lockfile", "Yes"),
                     ("has_dockerfile", "Yes"), ("has_contributing", "Yes")):
        seen = [r.get(key, "") for r in results if r.get(key) in ("Yes", "No")]
        agg[key] = (yes if any(v == yes for v in seen) else "No") if seen else ""
    agg["fork_parent"] = "; ".join(sorted(
        {r.get("fork_parent", "") for r in results if r.get("fork_parent")}))
    agg["build_files"] = "; ".join(sorted(
        {b for r in results for b in (r.get("build_files") or "").split("; ") if b}))
    agg["dependency_manifest"] = "; ".join(sorted(
        {r.get("dependency_manifest", "") for r in results
         if r.get("dependency_manifest")}))
    agg["review_basis"] = "; ".join(
        r["review_basis"] for r in results if r.get("review_basis"))
    stars = [r["stars"] for r in results if isinstance(r.get("stars"), int)]
    agg["stars"] = sum(stars) if stars else ""
...
def to_row(a):
    return {
        ...
        "Is fork": a.get("is_fork", ""),
        "Fork parent": a.get("fork_parent", ""),
        "Archived": a.get("archived", ""),
        "Stars": a.get("stars", ""),
        "First commit": a.get("first_commit", ""),
        "Last commit": a.get("last_commit", ""),
        "Active days": a.get("active_days", ""),
        "Active months": a.get("active_months", ""),
        "Commits per month": a.get("commits_per_month", ""),
        "Longest gap (days)": a.get("max_gap_days", ""),
        "Busiest day % of commits": a.get("pct_commits_busiest_day", ""),
        "Single drop": a.get("single_drop", ""),
        "Commit authors": a.get("commit_authors", ""),
        "Top author % of commits": a.get("top_author_pct", ""),
        "Author email domains": a.get("author_domains", ""),
        "Free-email commits %": a.get("pct_free_email_commits", ""),
        "Median commit msg length": a.get("median_commit_msg_len", ""),
        "Weak commit msgs %": a.get("pct_weak_commit_msgs", ""),
        "Conventional commits %": a.get("pct_conventional_commits", ""),
        "Commits with issue ref %": a.get("pct_commits_with_issue_ref", ""),
        "MRs reviewed %": a.get("pct_mrs_reviewed", ""),
        "Avg review comments": a.get("avg_review_comments", ""),
        "Median days to merge": a.get("median_days_to_merge", ""),
        "Review basis": a.get("review_basis", ""),
        "Direct dependencies": a.get("direct_dependencies", ""),
        "Dependency manifest": a.get("dependency_manifest", ""),
        "Lockfile": a.get("has_lockfile", ""),
        "Dockerfile": a.get("has_dockerfile", ""),
        "Build files": a.get("build_files", ""),
        "Pinned deps %": a.get("pct_pinned_deps", ""),
        "README words": a.get("readme_words", ""),
        "README code blocks": a.get("readme_code_blocks", ""),
        "Doc files": a.get("doc_files", ""),
        "CONTRIBUTING": a.get("has_contributing", ""),
        "Type hint % (py)": a.get("type_hint_pct", ""),
        "Minified files %": a.get("pct_minified_files", ""),
        "Non-ASCII comments %": a.get("nonascii_comment_pct", ""),
        "Conflicting license files": a.get("conflicting_license_files", ""),
    }
